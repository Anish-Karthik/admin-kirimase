...
export const createDepartment = async (department: NewDepartmentParams) => {
  ...
  const newDepartment = insertDepartmentSchema.parse({
    ...department,
...
export const updateDepartment = async (
  ...
  const newDepartment = updateDepartmentSchema.parse({
    ...department,
    userId: session?.user.id,
  });
  try {
    const d = await db.department.update({
...
export const deleteDepartment = async (id: DepartmentId) => {
  ...
  const { id: departmentId } = departmentIdSchema.parse({ id });
  try {
    const d = await db.department.delete({
      where: { id: departmentId, userId: session?.user.id },
    });
    return { department: d };
  } catch (err) {
