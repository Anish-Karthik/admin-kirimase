...
import {
  type CourseEnrolledStudentId,
  courseEnrolledStudentIdSchema,
} from "@/lib/db/schema/courseEnrolledStudent";

export const getCourseEnrolledStudent = async () => {
  const { session } = await getUserAuth();
  const c = await db.courseEnrolledStudent.findMany({
...
export const getCourseEnrolledStudentById = async (
