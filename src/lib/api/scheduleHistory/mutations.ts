...
export const createScheduleHistory = async (
  scheduleHistory: NewScheduleHistoryParams
) => {
  const { session } = await getUserAuth();
  const newScheduleHistory = insertScheduleHistorySchema.parse({
    ...scheduleHistory,
...
export const updateScheduleHistory = async (
  id: ScheduleHistoryId,
  scheduleHistory: UpdateScheduleHistoryParams
) => {
  const { session } = await getUserAuth();
  const { id: scheduleHistoryId } = scheduleHistoryIdSchema.parse({ id });
  const newScheduleHistory = updateScheduleHistorySchema.parse({
    ...scheduleHistory,
    userId: session?.user.id,
  });
  try {
    const s = await db.scheduleHistory.update({
...
export const deleteScheduleHistory = async (id: ScheduleHistoryId) => {
  ...
  const { id: scheduleHistoryId } = scheduleHistoryIdSchema.parse({ id });
  try {
    const s = await db.scheduleHistory.delete({
      where: { id: scheduleHistoryId, userId: session?.user.id },
    });
    return { scheduleHistory: s };
  } catch (err) {
