...
export const insertHolidayParams = baseSchema.extend({
  date: z.coerce.date(),
...
export const updateHolidayParams = updateHolidaySchema.extend({
  date: z.coerce.date(),
  collegeId: z.coerce.string().min(1)
}).omit({ 
  userId: true
...
// this type infers the return from getHoliday() - meaning it will include any joins
export type CompleteHoliday = Awaited<ReturnType<typeof getHoliday>>["holiday"][number];
