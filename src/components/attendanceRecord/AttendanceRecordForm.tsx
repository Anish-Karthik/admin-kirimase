...
const AttendanceRecordForm = ({
  ...
  const { data: enrollments } = trpc.enrollment.getEnrollment.useQuery();
  const { data: schedules } = trpc.schedule.getSchedule.useQuery();
  const { data: faculties } = trpc.faculty.getFaculty.useQuery();
  const { data: scheduleHistories } =
    trpc.scheduleHistory.getScheduleHistory.useQuery();
            ...
            <FormItem>
              ...
              <FormControl>
                ...
                >
                  ...
                  <SelectContent>
                    {enrollments?.enrollment.map((enrollment) => (
                      <SelectItem
                        key={enrollment.id}
                        value={enrollment.id.toString()}
                      >
                        {enrollment.id}{" "}
                        {/* TODO: Replace with a field from the enrollment model */}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
            ...
            <FormItem>
              ...
              <FormControl>
                ...
                >
                  ...
                  <SelectContent>
                    {scheduleHistories?.scheduleHistory.map(
                      (scheduleHistory) => (
                        <SelectItem
                          key={scheduleHistory.id}
                          value={scheduleHistory.id.toString()}
                        >
                          {scheduleHistory.id}{" "}
                          {/* TODO: Replace with a field from the scheduleHistory model */}
                        </SelectItem>
                      )
                    )}
                  </SelectContent>
