...
const EnrollmentForm = ({
  ...
  const form = useForm<z.infer<typeof insertEnrollmentParams>>({
  ...
  const handleSubmit = (values: NewEnrollmentParams) => {
    if (editing) {
      updateEnrollment({ ...values, id: enrollment.id });
    } else {
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className={"space-y-8"}>
        ...
        {editing ? (
          <Button
            type="button"
            variant={"destructive"}
            onClick={() => deleteEnrollment({ id: enrollment.id })}
          >
